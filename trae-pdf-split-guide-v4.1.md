...
# Trae PDF 分片工作流指南（v4.2）

> 本指南记录了 PDF 按章节/小节分片的完整工作流、核心要点和常见陷阱，供其他 Trae 对话复用。
> 基于 **18 本**教材/参考书的实际分片经验迭代优化。

> [!tip] v4.2 主要更新
> - 新增 **§2.8 大规模 null page 书签处理**：页眉文本提取作为精确页码来源，等分推断策略为何失败
> - 新增 **§2.9 书签与正文不一致的三种场景**：幽灵节过滤、遗漏节补充、页码覆盖修正
> - 增强 **§2.4.1**：新增页眉提取方法（区别于正文提取），可靠性优势说明
> - 新增 **§6.4 分片粒度调整策略**：L2→L1 降级规则、L1 含 L2 子节的处理
> - 新增 **§10.3 零填充编号规范**：`01_`/`01.01`/`01.001` 三级零填充保证文件系统排序
> - 新增 **陷阱 21-24**：大规模 null page、幽灵节、遗漏节、文件名排序不一致
> - 增强 **§十一 验证清单**：页眉校验、幽灵节检测、遗漏节检测
> - 更新 **已处理书籍清单**（18 本）
> - 新增 **附录 F**：现代操作系统第5版分片实战记录
> - 新增 **附录 G**：数据结构C++语言版第3版分片实战记录（含完整复盘）
>
> [!tip] v4.1 更新（保留）
> - 新增 **§2.7 书签层级修正**：三种典型层级不一致场景及通用检测修正流程
> - 新增 **§6.2 跨章假重叠检测与消除**：跨章边界精确切割，消除约 28 处假重叠
> - 新增 **§6.3 书签未覆盖内容的独立分片**：页面全覆盖检查，Part 标题页/Problems 补充分片
> - 新增 **§10.2 特殊内容目录归属规则**：Problems、Part 标题页、篇引言等的目录归属
> - 新增 **陷阱 16-20**：书签层级不一致、跨章假重叠、书签未覆盖内容、无独立章标题书签、特殊内容目录归属不一致
> - 新增 **附录 D**：算法导论第四版分片实战记录
> - 新增 **附录 E**：深入理解计算机系统第3版分片实战记录
> - 增强 **§2.4.1**：特殊前缀处理、非标准标题识别
> - 增强 **§六 重叠页决策树**：区分同章小节与跨章边界
> - 增强 **§九 陷阱 2**：从"书签层级过深"扩展为"书签层级不一致"
> - 增强 **§九 陷阱 15**：无书签的 Part 标题页处理
> - 增强 **§八 模板 C 注意事项**：书签层级检查、页面全覆盖、跨章边界不重叠
> - 增强 **§十一 验证清单**：页面全覆盖检查、跨章边界无假重叠
...
> - **特殊前缀处理**（v4.1 新增）：某些 PDF 的节标题可能带有特殊符号前缀（如 CLRS4 附录 C.5 前缀有 `★` 符号），正则需要适配：`^(★\s*)?[A-D]\.\d+\s+(.{2,40})$`
> - **非标准标题识别**（v4.1 新增）：某些书签标题不遵循 `X.Y 标题` 格式，如"参考答案、练习题答案"、"家庭作业"。这些内容虽然不是标准小节，但需要作为独立分片归入对应章节（见 §2.7）
> - **页眉提取方法**（v4.2 新增）：除了正文中的节标题，还可以从页面页眉提取节编号。页眉提取比正文提取更可靠——每页都有页眉标注，不受正文内容干扰。详见 §2.8
...
> [!warning] 书签层级不等于逻辑层级
> PDF 书签的编码层级（L0/L1/L2）可能与书籍的逻辑层级（篇/章/节）不一致，必须逐级检查。

**三种典型层级不一致场景：**

| 场景 | 表现 | 实战案例 | 修正方法 |
|------|------|---------|---------|
| 附录子节偏高 | 附录 A.x/B.x/C.x/D.x 被编码为 L1（与章同级） | CLRS4：A.1~D.2 为 L1 | 正则 `^(★\s*)?[A-D]\.\d+` 识别为有效 L2 |
| 首章小节偏高 | 首章小节被编码为 L1（因缺少独立章标题书签） | CSAPP3：1.1-1.10 为 L1 | 正则 `^\d+\.\d+` 识别为有效 L2 |
| 章内附属偏高 | 练习题答案等附属内容被编码为 L1 | CSAPP3："参考答案、练习题答案"为 L1 | 标题匹配"练习题答案"/"家庭作业"则提升为有效 L2 |

**通用检测与修正流程：**

```python
# 步骤 1：打印完整书签结构，人工审查层级异常
for title, page, level in bookmarks:
    print(f"  L{level} p{page}: {title}")

# 步骤 2：在分片脚本中应用层级修正
for bm in bookmarks:
    if bm["level"] == 1 and re.match(r'^\d+\.\d+', bm["title"]):
        bm["eff_level"] = 2  # 首章小节 → 有效 L2
    elif bm["level"] == 1 and re.match(r'^(★\s*)?[A-D]\.\d+', bm["title"]):
        bm["eff_level"] = 2  # 附录子节 → 有效 L2
    elif bm["level"] == 1 and ("练习题答案" in bm["title"] or "家庭作业" in bm["title"]):
        bm["eff_level"] = 2  # 章内附属 → 有效 L2
    else:
        bm["eff_level"] = bm["level"]
```

**关键原则**：分片前必须打印完整书签结构并人工审查，不能假设 L0=篇、L1=章、L2=节。

### 2.8 大规模 null page 书签处理（v4.2 新增）

> [!warning] 某些 PDF 的书签大部分没有页码
> DS3（数据结构第3版）的 52 个正文节中，34 个书签页码为 None（65%）。这意味着仅靠书签无法完成分片。

**null page 书签的成因**：
- PDF 制作工具（如 FreePic2Pdf）生成书签时未设置跳转目标
- 扫描版 PDF 的书签可能是手动添加的，忘记关联页码
- 某些 PDF 编辑器导出时丢失了书签的 destination 信息

**已尝试并失败的推断策略**（DS3 实战复盘）：

| 策略 | 思路 | 失败原因 |
|------|------|---------|
| 子树搜索 | 在书签子树中找最近的有效页码条目 | 子树内所有条目也是 null page |
| 两遍扫描 | 第一遍处理有效页码条目，第二遍填充 null | 有效条目的结束页延伸到章边界，null 条目无空间 |
| 统一锚点推断 | 所有条目按原始顺序排列，null 条目从相邻锚点推断 | 连续 null 条目获得相同页码范围 |
| 等分页码空间 | 将连续 null 条目之间的页码空间平分 | 内容长度不均匀，等分完全不匹配实际 |

**唯一可靠方案：页眉文本提取**

> [!tip] 页眉是分片边界的"地面真相"
> 教材 PDF 的每页页眉通常标注当前节的编号和标题（如 `§3.2 接口`），这是最可靠的节边界信息来源。
...
SECTION_PAGES = {}  # 节编号 -> 首次出现的 PDF 页码（1-based）
    ...
    for line in text.split('\n')[:5]:
        ...
        if m:
            ...
            if sec_num not in SECTION_PAGES:
                SECTION_PAGES[sec_num] = p + 1  # 1-based
                print(f"  §{sec_num} -> p{p+1}")
            break  # 每页只取第一个匹配
```

**在分片脚本中使用 SECTION_PAGES 覆盖 null page 书签**：

```python
# 用页眉提取的精确页码覆盖 null page 书签
for bm in bookmarks:
    if bm["page"] is None and bm["level"] == 1:
        m = re.match(r'^§\s*(\d+\.\d+)', bm["title"])
        if m and m.group(1) in SECTION_PAGES:
            bm["page"] = SECTION_PAGES[m.group(1)]
```

**页眉提取的注意事项**：
- 某些节可能只有 1 页，页眉直接切换到下一节（如 §6.5 与 §6.6 共用 p109）
- 选读节（带 `*` 前缀）可能不在页眉中出现（内容被合并到前一节）
- 页眉格式因书而异，需先查看几页确认格式再编写正则

### 2.9 书签与正文不一致的三种场景（v4.2 新增）

> [!important] 书签 ≠ 目录 ≠ 正文
> 分片时必须以正文实际内容为准，书签和目录可能有遗漏或冗余。

**场景 A：幽灵节（书签有但正文无）**

某些节在书签和目录中列出，但 PDF 正文中没有对应内容。常见于：
- 不同版本/印次的节号差异（如第3版删除了第2版的某些节）
- 选读节（带 `*` 前缀）被省略
- 章节编号跳跃（如 §4.4 和 §4.5 不存在，直接从 §4.3 跳到 §4.6）

**DS3 实战案例**：16 个幽灵节（§1.4, §2.7, §2.8, §3.4, §3.5, §4.4, §4.6, §5.5, §6.11, §6.12, §8.3, §8.4, §9.4, §10.3, §11.5, §12.3），占书签节总数的 31%。

**检测方法**：对比书签列表与页眉提取结果，页眉中从未出现的节即为幽灵节。

```python
# 检测幽灵节
bookmark_sections = set()
for bm in bookmarks:
    m = re.match(r'^§\s*(\d+\.\d+)', bm["title"])
    if m:
        bookmark_sections.add(m.group(1))

header_sections = set(SECTION_PAGES.keys())
ghost_sections = bookmark_sections - header_sections
if ghost_sections:
    print(f"⚠ 幽灵节（书签有但页眉无）: {sorted(ghost_sections)}")
```

**处理**：在分片脚本中过滤掉幽灵节，避免生成虚假分片。

```python
NONEXISTENT_SECTIONS = {"1.4", "2.7", "2.8", ...}  # 幽灵节集合

def _is_nonexistent(entry):
    title = entry.get("title", "")
    m = re.match(r'^§\s*(\d+\.\d+)', title)
    return m.group(1) in NONEXISTENT_SECTIONS if m else False

resolved = [e for e in all_entries if not _is_nonexistent(e)]
```

**场景 B：遗漏节（正文有但书签无）**

某些节在正文中存在（页眉有标注），但书签中缺失。常见于：
- PDF 制作时遗漏了某些书签条目
- 章节编号不连续时，跳过的编号对应的书签被删除

**DS3 实战案例**：3 个遗漏节（§6.4 邻接表、§6.8 拓扑排序、§11.2 蛮力算法）。

**检测方法**：对比页眉提取结果与书签列表，页眉中有但书签中无的节即为遗漏节。

```python
missing_sections = header_sections - bookmark_sections
if missing_sections:
    print(f"⚠ 遗漏节（页眉有但书签无）: {sorted(missing_sections)}")
```

**处理**：为遗漏节补充虚拟书签，使其参与正常分片流程。
...
for sec_num, sec_title in MISSING_SECTIONS.items():
    sec_page = SECTION_PAGES[sec_num]
    # 在正确的位置插入虚拟书签
    bookmarks.append({
        "title": sec_title, "page": sec_page,
        "level": 1, "eff_level": 1, "_virtual": True,
    })
```

**场景 C：书签页码与实际不符**

某些书签有页码但指向的页面不正确（如指向节标题页而非内容起始页）。

**检测方法**：对比书签页码与页眉提取的页码，不一致的以页眉为准。
...
for bm in bookmarks:
    if bm["page"] is not None:
        m = re.match(r'^§\s*(\d+\.\d+)', bm["title"])
        if m and m.group(1) in SECTION_PAGES:
            if bm["page"] != SECTION_PAGES[m.group(1)]:
                print(f"⚠ 页码不一致: {bm['title']} 书签p{bm['page']} -> 页眉p{SECTION_PAGES[m.group(1)]}")
                bm["page"] = SECTION_PAGES[m.group(1)]  # 以页眉为准
```

**综合检测流程**：

```
1. 提取书签列表 → bookmark_sections
2. 提取页眉列表 → header_sections
3. 幽灵节 = bookmark_sections - header_sections → 过滤
4. 遗漏节 = header_sections - bookmark_sections → 补充
5. 页码不一致 = 交集但页码不同 → 覆盖
```
...
### 6.2 跨章假重叠检测与消除（v4.1 新增）

> [!important] 重叠页只适用于同章相邻小节
> 跨章/跨 Part 边界处，下一章标题页只属于新章，不包含前一节内容。如果盲目重叠，会产生"假重叠"——边界页实际只属于新章，前一节并没有内容延伸到该页。

**问题描述**：
- CLRS4 实战发现约 28 处跨章假重叠
- 例如：1.2 节结束于 p43，第2章标题页在 p44，如果 1.2 的 end_page = p44，则 p44（第2章标题页）被错误地包含在 1.2 的分片中

**判断规则**：

```python
# L2 小节结束页计算时，检查下一个条目的有效层级
if eff == 2:  # 当前是 L2 小节
    next_eff = entries[j]["eff_level"]
    if next_eff <= 1:  # 下一条目是章标题(L1)或Part标题(L0)
        end_page = entries[j]["page"] - 1  # 精确切割，不重叠
    else:
        end_page = entries[j]["page"]  # 同章小节间保留 1 页重叠
```

**验证方法**：
- 分片后统计总重叠页数，如果远超预期（如 149 vs 预期 93），说明存在假重叠
- 抽查跨章边界处的分片文件末页，确认不包含下一章标题

### 6.3 书签未覆盖内容的独立分片（v4.1 新增）

> [!warning] 书签不等于全书结构
> 即使有完整书签，也可能遗漏某些内容（如章末习题、Part 标题页）。这些内容不在书签中，但不应丢失。

**三种典型遗漏**：

| 遗漏类型 | CLRS4 实战 | CSAPP3 实战 | 处理方式 |
|---------|-----------|-----------|---------|
| **章末 Problems** | 35 章 Problems 无书签 | 有独立书签（"家庭作业"） | 无书签时需 OCR + Read 手动定位 |
| **Part 标题页** | 8 个 Part 首页有书签 | 3 个 Part 标题页(p57/p499/p656)无书签 | 无书签时加入辅助材料 |
| **章标题页** | 部分章有独立标题页 | 第6/10/11章标题与首小节同页 | 有则独立分片，无则不生成 |
...
if missing:
    print(f"⚠ 遗漏页码: {missing}")
    # 用 Read 工具查看遗漏页内容，归入辅助材料或对应章节
```

**章末 Problems 的定位方法**（扫描版 PDF，无书签时）：

```python
# 方法：OCR + Read 工具逐章扫描，找到 "Problems" 粗体标题页
# 硬编码到脚本中：
PROBLEMS_PAGES = {
    1: 41, 2: 78, 3: 112, ...  # 章: Problems起始页
}
```

### 6.4 分片粒度调整策略（v4.2 新增）

> [!note] 初始分片粒度不一定合适
> 实战中经常需要调整分片粒度。MOS5 首次 L2(x.x.x) 分片产生 410 文件，用户要求改为 L1(x.x) 节级 → 125 文件。

**何时需要调整粒度**：

| 情况 | 建议 | 实战案例 |
|------|------|---------|
| 文件数过多（>200） | 考虑降低一级粒度 | MOS5：410(L2) → 125(L1) |
| 某级内容太碎（<3页/文件） | 合并到上一级 | MOS5 L2 小节很多只有 1-2 页 |
| 用户明确要求 | 按用户要求调整 | MOS5 用户要求仅分到 x.x |

**L1 节含 L2 子节时的处理规则**：

当从 L2 降级到 L1 时，L1 节如果有 L2 子节，需要特殊处理：

```python
# L1 节的处理规则
if eff == 1 and is_section_title(title):
    # 检查是否有 L2 子节
    has_children = any(
        entries[j]["eff_level"] == 2
        for j in range(i + 1, len(entries))
        if entries[j]["page"] <= end_page
    )
    if has_children:
        # L1 节有 L2 子节：不作为独立分片，由 L2 子节覆盖其内容
        continue
    else:
        # L1 节无 L2 子节：作为独立分片
        chunks.append(...)
...
**粒度调整对重叠策略的影响**：
- L2→L1 降级后，重叠页数量减少（L1 节数远少于 L2 小节数）
- 跨章边界处理不变（仍然精确切割）
...
> - **必须检查书签层级一致性**（v4.1 新增）：不能假设 L0=篇、L1=章、L2=节（见 §2.7）
> - **必须检查页面全覆盖**（v4.1 新增）：书签未覆盖的页面（如 Part 标题页、章末 Problems）需手动补充（见 §6.3）
> - **跨章边界不重叠**（v4.1 新增）：同章小节间保留 1 页重叠，跨章边界精确切割（见 §6.2）
...
### 陷阱 2：书签层级不一致（v4.1 增强）

**表现**：
- v4.0 原描述：所有节和小节都在顶层（扁平书签），导致拆分出 50+ 个文件。
- v4.1 新增：书签层级与书籍逻辑层级不匹配。

| 问题 | 表现 | 实战案例 |
|------|------|---------|
| 扁平书签 | 所有节都在顶层 | v4.0 已记录 |
| 附录层级偏高 | 附录子节被编码为章同级 | CLRS4：A.x ~ D.x 为 L1 而非 L2 |
| 首章小节偏高 | 首章小节被编码为章同级 | CSAPP3：1.1-1.10 为 L1 |
| 章标题缺失 | 某章无独立书签，小节直接暴露 | CSAPP3：第1章无章标题书签 |

**解决**：分片前打印完整书签结构并逐级审查，不能假设固定的层级映射关系。详见 §2.7。
...
- 第一章从篇首页之后正式开始
>
> **无书签的 Part 标题页**（v4.1 新增）：
>
> 某些 PDF 的 Part 标题页没有对应书签（如 CSAPP3 的 p57、p499、p656），导致这些页面在分片后"丢失"——不属于任何分片文件。
>
> **检测方法**：分片完成后，对比所有分片覆盖的页码集合与全书页码集合，找出遗漏页码（见 §6.3）。
>
> **处理**：将遗漏的 Part 标题页加入辅助材料目录。

### 陷阱 16：书签层级与逻辑层级不一致（v4.1 新增）

**表现**：按书签层级分片后，发现附录子节被分到 Part 目录下（与章同级），或首章小节被当作独立章处理。

**原因**：PDF 制作工具对书签层级的编码不一定反映书籍的真实逻辑结构。

**实战案例**：
- CLRS4：附录 C.5（`★ C.5 The tails of the binomial distribution`）被编码为 L1，与章同级
- CSAPP3：第1章的 1.1-1.10 小节被编码为 L1，因为第1章缺少独立的章标题书签

**解决**：
1. 分片前打印完整书签结构，逐级审查
2. 用正则匹配识别"错位"条目（如 L1 中的 `X.Y` 格式、`A.X` 格式）
3. 在分片脚本中添加层级修正逻辑（见 §2.7）

### 陷阱 17：跨章假重叠（v4.1 新增）

**表现**：分片后总页数远超预期（如 149 处重叠 vs 预期 93 处），抽查发现某些分片文件末页包含下一章的标题页。

**原因**：重叠策略未区分"同章相邻小节"和"跨章边界"，导致跨章处也产生了 1 页重叠。

**实战案例**：CLRS4 首次重叠优化后，1.2 节的 end_page = p44（第2章标题页），但 p44 实际只属于第2章，不包含 1.2 的内容。

**解决**：
```python
if next_eff <= 1:  # 下一条目是章标题或 Part 标题
    end_page = entries[j]["page"] - 1  # 精确切割
else:
    end_page = entries[j]["page"]  # 同章小节间重叠
```

### 陷阱 18：书签未覆盖内容导致页面丢失（v4.1 新增）

**表现**：分片完成后，对比页码发现若干页面不属于任何分片文件。

**原因**：书签不覆盖全书所有内容。常见遗漏：
- Part 标题页（尤其是无书签的）
- 章末 Problems / Chapter Notes（无书签时）
- 章标题页（如果与首小节不同页但无独立书签）

**实战案例**：
- CLRS4：35 章 Problems 无书签，8 个 Part 首页虽有书签但需独立处理
- CSAPP3：3 个 Part 标题页（p57/p499/p656）无书签，分片后丢失

**解决**：
1. 分片后执行页面全覆盖检查（见 §6.3）
2. 对遗漏页码逐个用 Read 工具查看内容
3. 根据内容类型归入辅助材料或对应章节文件夹

### 陷阱 19：无独立章标题书签导致目录推断困难（v4.1 新增）

**表现**：某些章节没有独立的章标题书签，导致无法确定该章的目录文件夹名。

**原因**：PDF 制作时，某些章的标题与第一个小节在同一页上，只生成了小节书签而未生成章标题书签。

**实战案例**：
- CSAPP3 第1章：无独立章标题书签，1.1-1.10 直接暴露在 L1 层级
- CSAPP3 第6/10/11章：章标题与首小节同页，不生成独立标题页分片

**解决**：
1. **目录推断**：向前搜索相邻条目中第一个 `X.Y` 格式的条目，推断章编号，生成目录名（如 `第1章_计算机系统漫游/`）
2. **标题页处理**：如果章标题与首小节同页，不生成独立标题页分片（内容已包含在首小节中）
3. **验证**：确认推断的目录名与书籍目录页一致

### 陷阱 20：特殊内容目录归属不一致（v4.1 新增）

**表现**：Problems 文件被放到缺少章号前缀的独立文件夹下（如 `Getting_Started/Problems.pdf`），而对应小节在带章号的文件夹下（如 `2_Getting_Started/`）。

**原因**：Problems 的目录名生成逻辑与小节不同——小节通过书签的父节点获取章目录名，而 Problems 作为手动添加的分片条目，可能使用了不同的命名逻辑。

**实战案例**：CLRS4 首次分片时，Problems 使用书签标题（如 `Getting Started`）作为目录名，缺少章号前缀 `2_`。

**解决**：
1. Problems 的目录名必须与小节共享同一个章节目录
2. 通过查找 Problems 所属章的第一个小节书签，获取完整的章节目录名（含章号前缀）
3. 分片后检查：每个 `Problems.pdf` 是否与对应小节在同一文件夹下

### 陷阱 21：大规模 null page 书签导致等分推断失败（v4.2 新增）

**表现**：分片后大量边界错误，用户反馈"1.3 分片错误"、"2.5 分片错误"等。

**原因**：当超过 50% 的书签页码为 None 时，等分页码空间的策略完全不适用——不同节的内容长度差异巨大（1 页到 13 页不等），等分产生的边界与实际严重不符。

**实战案例**：DS3 的 52 个节中 34 个 null page（65%），等分推断后 §1.3 被分配 2 页（实际 4 页），§5.3 被分配 3 页（实际 8 页）。

**解决**：
1. 当 null page 比例超过 30% 时，放弃推断策略
2. 改用页眉文本提取获取精确页码（见 §2.8）
3. 构建硬编码的 `SECTION_PAGES` 字典覆盖 null page 书签

### 陷阱 22：书签中的幽灵节（v4.2 新增）

**表现**：分片结果中出现了内容完全不对的文件（如 §1.4 的内容实际上是 §1.3 的延续）。

**原因**：某些节在书签和目录中列出，但 PDF 正文中没有对应内容（可能是不同版本的节号差异）。如果不过滤，这些幽灵节会抢占相邻真实节的页码空间。

**实战案例**：DS3 有 16 个幽灵节（占 31%），导致 §1.3 被截断为 2 页（实际应为 4 页），§4.3 被截断为 2 页（实际应为 7 页）。

**解决**：
1. 用页眉提取结果与书签对比，识别幽灵节
2. 在分片脚本中定义 `NONEXISTENT_SECTIONS` 集合并过滤（见 §2.9 场景 A）
3. 过滤后相邻真实节的边界自动延伸到正确位置

### 陷阱 23：书签遗漏节导致分片缺失（v4.2 新增）

**表现**：分片完成后，某些节的 PDF 文件不存在，但该节在正文中确实有内容。

**原因**：PDF 制作时遗漏了某些书签条目。这些节在页眉中有标注，但书签中缺失。

**实战案例**：DS3 的 §6.4（邻接表）、§6.8（拓扑排序）、§11.2（蛮力算法）在书签中缺失，导致分片时被遗漏。

**解决**：
1. 用页眉提取结果与书签对比，识别遗漏节
2. 为遗漏节补充虚拟书签条目（见 §2.9 场景 B）
3. 虚拟书签的页码从 `SECTION_PAGES` 字典获取

### 陷阱 24：文件名排序与逻辑排序不一致（v4.2 新增）

**表现**：在文件管理器中，`10_第10章` 排在 `1_第1章` 前面，`2.10_排序器` 排在 `2.1_从数组到向量` 和 `2.2_接口` 之间。

**原因**：默认的 ASCII/字典序排序中，`1` < `10` < `2`（因为 `1` 和 `0` 的 ASCII 码都小于 `2`）。

**实战案例**：MOS5 首次分片后，12 个章节文件夹在文件管理器中顺序混乱。

**解决**：使用零填充编号（见 §10.3），确保 `01_` < `02_` < ... < `10_` < `11_` < `12_`。
...
### 10.2 特殊内容的目录归属规则（v4.1 新增）

> [!note] 不是所有内容都按书签层级归目录
> 某些特殊内容的目录归属需要根据语义而非书签层级决定。

**归属规则表**：

| 内容类型 | 归属位置 | 理由 |
|---------|---------|------|
| **章末 Problems** | 对应章节文件夹下（如 `Ch2_Getting_Started/Problems.pdf`） | Problems 是该章的习题，应与小节放在一起 |
| **章末 Chapter Notes** | 与 Problems 合并或单独放在章节文件夹下 | 同上 |
| **Part 标题页** | 辅助材料目录（`00_辅助材料/Part_X_篇名.pdf`） | Part 标题页不属于任何一章 |
| **篇引言（Introduction）** | 对应 Part 目录下（如 `Part_I/Introduction.pdf`） | 篇引言属于该 Part |
| **每章末尾的"参考文献说明"** | 对应章节文件夹下 | 属于该章的附属内容 |
| **每章末尾的"练习题答案"** | 对应章节文件夹下 | 属于该章的附属内容 |

**CLRS4 经验教训**：Problems 最初被放到缺少章号前缀的独立文件夹（如 `Getting_Started/Problems.pdf`），而对应小节在 `2_Getting_Started/` 下，导致目录不一致。修复后 Problems 与小节共享同一章节文件夹。

### 10.3 零填充编号规范（v4.2 新增）

> [!warning] 文件名排序 ≠ 逻辑排序
> 默认的文件系统排序（ASCII/字典序）会导致 `10_` 排在 `1_` 前面，`2.5` 排在 `2.10` 前面。
...
| 层级 | 格式 | 示例 |
|------|------|------|
| 章级 | `XX_` | `01_第1章_绪论`、`10_第10章_优先级队列` |
| 节级 | `XX.XX_` | `01.01_计算机与算法`、`02.10_排序器` |
| 小节级 | `XX.XX.XXX_` | `01.01.001_概述`、`01.01.010_总结` |
...
def sanitize(title):
    ...
    name = re.sub(r'^\*\s*', '', name)
    # 节编号零填充：1.1 → 01.01
    name = re.sub(r'^(\d+)\.(\d+)', lambda m: f"{int(m.group(1)):02d}.{int(m.group(2)):02d}", name)
...
def chapter_dir_name(title):
    ...
    if m:
        ...
        return f"{ch_num:02d}_{sanitize(m.group(1))}_{sanitize(ch_title)}"
    return sanitize(title)
```

**MOS5 实战案例**：首次分片使用无零填充编号，`10_` 排在 `1_` 前面。修复后使用 `01_`/`01.01` 格式，文件系统排序与逻辑排序完全一致。
...
- [ ] 无重复文件夹（v4.0 新增）
- [ ] **页面全覆盖检查**（v4.1 新增）：所有分片的页码并集应等于全书页码集合，无遗漏页（见 §6.3）
- [ ] **跨章边界无假重叠**（v4.1 新增）：抽查跨章边界处的分片文件末页，确认不包含下一章标题（见 §6.2）
- [ ] **页眉校验**（v4.2 新增）：每个分片的起始页应与 PDF 页眉中该节首次出现的页码一致
- [ ] **幽灵节检测**（v4.2 新增）：确认书签中所有节在正文中有对应内容（页眉中出现过），过滤幽灵节（见 §2.9 场景 A）
- [ ] **遗漏节检测**（v4.2 新增）：确认页眉中所有节在分片清单中存在，补充遗漏节（见 §2.9 场景 B）
- [ ] **文件名排序正确**（v4.2 新增）：确认文件系统中的排序与逻辑排序一致（使用零填充编号，见 §10.3）
...
| 15 | 算法导论 第四版 (CLRS) | 1676 | 按小节(135)+Problems(35) | 无 | 书签(208个,3级)+OCR | 扫描版 | 227 | — |
| 16 | 深入理解计算机系统 第3版 (CSAPP) | 775 | 按小节(139) | 无 | 书签(175个,3级)+层级修正 | 文本版 | 169 | — |
| 17 | Modern Operating Systems 5th (MOS5) | 1185 | 按节(110) | 无 | 书签(492个,3级) | 扫描版 | 125 | — |
| 18 | 数据结构 C++语言版 第3版 (DS3) | 238 | 按节(44) | 无 | 书签(330个)+页眉提取+幽灵节过滤 | 扫描版 | 64 | — |

> [!note] 部署概况（截至 v4.2）
> - 已完成分片的书籍：**10 本**（#8-#18），共约 **1301 个 PDF 文件**
> - 新增经验：大规模 null page 书签处理、页眉精确页码提取、幽灵节过滤、遗漏节补充、零填充编号
> - 涵盖 PDF 类型：扫描版(5本)、文本版(5本)、合并PDF(1本)
> - 涵盖书签情况：完整书签(6本)、无用书签(1本)、无书签(2本)、大规模null page书签(1本)
...
## 附录 D：实战记录 — 算法导论第四版分片（v4.1 新增）

### 基本信息

- **PDF**：Introduction to Algorithms (CLRS) 4th Edition
- **总页数**：1676 页
- **PDF 类型**：扫描版（无文本层），但有完整书签（208 个，3 级层级）
- **分片粒度**：按小节（含 Problems 独立分片）

### 书签结构

- L0: 14（封面、目录、各 Part、附录等）
- L1: 35（各章标题 + 附录子节 A.x~D.x）
- L2: 139（各章小节）
- 特殊：附录子节被编码为 L1（与章同级），需层级修正

### 遇到的问题与解决方案

| # | 问题 | 解决方案 |
|---|------|---------|
| 1 | 附录层级修正 | 正则 `^(★\s*)?[A-D]\.\d+` 识别附录子节，降为有效 L2 |
| 2 | ★ 符号前缀 | 附录 C.5 前缀有 ★ 符号，正则需适配 |
| 3 | Problems 无书签 | 35 章 Problems 需 OCR + Read 手动定位，硬编码到脚本 |
| 4 | 跨章假重叠 | 约 28 处，通过检查 `next_eff <= 1` 消除 |
| 5 | Part 标题页 | 8 个 Part 首页加入辅助材料 |
| 6 | Problems 目录归属 | 修正为对应章节文件夹下（含章号前缀） |

### 最终结果

- 227 个文件（135 小节 + 35 Problems + 8 Part 首页 + 8 篇引言 + 若干章标题页 + 辅助材料）
- 1851 页（含 175 页重叠）
- 0 页遗漏，0 处假重叠

---

## 附录 E：实战记录 — 深入理解计算机系统第3版分片（v4.1 新增）

### 基本信息

- **PDF**：CSAPP 3rd Edition（中文扫描版可编辑）
- **总页数**：775 页
- **PDF 类型**：文本版（可提取文本）
- **书签**：175 个（L0:14, L1:22, L2:139）

### 书签结构特点

- **第1章特殊**：小节 1.1-1.10 编码为 L1（与章同级），需层级修正
- **第2-12章正常**：章标题 L1，小节 L2
- **每章末尾**：小结、参考文献说明、家庭作业、练习题答案（均为 L2）
- **3 个 Part**：Part I(p57)、Part II(p499)、Part III(p656)

### 遇到的问题与解决方案

| # | 问题 | 解决方案 |
|---|------|---------|
| 1 | 第1章层级异常 | `^\d+\.\d+` 识别 1.1-1.10 为有效 L2 |
| 2 | 第1章练习题答案 | "参考答案、练习题答案"为 L1，特殊识别并提升为 L2 |
| 3 | 第1章无独立章标题 | 根据 1.x 编号推断章节目录 `第1章_计算机系统漫游/` |
| 4 | Part 标题页缺失 | p57/p499/p656 无书签，加入辅助材料 |
| 5 | 第6/10/11章无标题页 | 章标题与首小节同页，不生成独立标题页分片 |

### 最终结果

- 171 个文件（139 小节 + 12 章节末内容 + 9 第1章内容 + 3 Part 标题页 + 13 辅助材料）
- 913 页（含 138 页重叠）
- 775/775 页全覆盖，0 页遗漏，0 处假重叠

---

## 附录 F：实战记录 — 现代操作系统第5版分片（v4.2 新增）

### 基本信息

- **PDF**：Modern Operating Systems 5th Edition (MOS5)
- **总页数**：1185 页
- **PDF 类型**：扫描版（无文本层），有完整书签（492 个，3 级层级）
- **分片粒度**：按节（x.x），零填充编号
...
1. **首次 L2 分片**：按 x.x.x 小节级分片，产生 410 个文件
2. **用户要求调整**：仅分到 x.x 节级
3. **粒度调整**：移除 L2 处理，只保留 L1 节标题 → 125 个文件
4. **零填充修复**：`10_` 排在 `1_` 前面 → 改为 `01_`/`01.01` 格式

### 遇到的问题与解决方案

| # | 问题 | 解决方案 |
|---|------|---------|
| 1 | L2 分片文件过多（410个） | 降级到 L1 节级（125个） |
| 2 | L1 节含 L2 子节时重复分片 | 检查 `_has_l2_children`，有子节则跳过 L1 |
| 3 | 文件名排序混乱 | 零填充编号 `01_`/`01.01` |

### 最终结果

- 125 个文件（110 节 + 13 章标题页 + 6 辅助材料）
- 1185/1185 页全覆盖，307 页重叠

---

## 附录 G：实战记录 — 数据结构C++语言版第3版分片（v4.2 新增）

### 基本信息

- **PDF**：数据结构（C++语言版）第3版（邓俊辉）
- **总页数**：238 页
- **PDF 类型**：扫描版（有文本层，可提取页眉文本）
- **书签**：330 个（含习题解析部分），但 65% 的正文节页码为 None
- **分片粒度**：按节（§x.y），零填充编号

### 书签结构特点

- **大规模 null page**：52 个正文节中 34 个书签页码为 None
- **幽灵节**：16 个节仅在书签/目录中出现，正文无对应内容
- **遗漏节**：3 个节在页眉中存在但书签中缺失
- **页眉标注**：每页页眉标注当前节编号（如 `§3.2 接口`），可作为精确边界来源

### 分片策略演进（完整复盘）

DS3 的分片经历了 **5 次迭代**，每次失败都揭示了新的问题：

| 迭代 | 策略 | 结果 | 失败原因 |
|------|------|------|---------|
| 1 | 子树搜索推断 null page | 57 节中 32 节丢失 | 子树内所有条目也是 null page |
| 2 | 两遍扫描（先有效后 null） | null 节全部跳过 | 有效条目结束页延伸到章边界，null 条目无空间 |
| 3 | 统一排序 + 锚点推断 | 78 文件，边界大面积错误 | 连续 null 条目获得相同页码范围 |
| 4 | 等分页码空间 | 78 文件，用户反馈 12 处错误 | 内容长度不均匀（1-13 页），等分不匹配 |
| 5 | **页眉提取 + 幽灵节过滤** | **64 文件，全部 44 节验证通过** | — |

### 关键经验

**经验 1：页眉是分片边界的"地面真相"**

DS3 的每页页眉标注当前节编号，通过逐页提取页眉文本，构建了包含 52 个精确页码映射的 `SECTION_PAGES` 字典。这是唯一可靠的方法——所有基于书签的推断策略都失败了。

**经验 2：幽灵节必须过滤，否则会"污染"相邻分片**

16 个幽灵节（如 §1.4、§2.7）在书签中存在但正文中无内容。如果不过滤，它们会抢占相邻真实节的页码空间，导致 §1.3 被截断 2 页、§4.3 被截断 5 页。

**经验 3：遗漏节必须补充，否则内容会丢失**

3 个遗漏节（§6.4、§6.8、§11.2）在页眉中存在但书签中缺失。通过补充虚拟书签，确保所有正文内容都被分片覆盖。

**经验 4：null page 比例超过 30% 时，放弃推断策略**

DS3 的 null page 比例为 65%，远超阈值。此时等分推断、锚点推断等策略都不可靠，应直接使用页眉提取。

### 最终结果

- 64 个文件（44 节 + 11 章标题页 + 9 辅助材料）
- 268 页（含 30 页重叠）
- 238/238 页全覆盖，0 页遗漏
- 全部 44 个节起始页与 PDF 页眉完全一致
